...
void resolver_init_phase(void) {
    ...
    if (phase4_count > 0) {
        ...
        int mtu_wait_ms = (g_cfg.mtu_test_timeout_ms > 0) ? g_cfg.mtu_test_timeout_ms * 10 : 10000;
    ...
    for (int i = 0; i < g_pool.count; i++) {
        ...
        if (results[i].longname_supported && results[i].nxdomain_correct && 
            (results[i].edns_supported || results[i].txt_supported)) {
            if (results[i].up_mtu_search.optimal > 0) r->upstream_mtu = results[i].up_mtu_search.optimal;
            else if (results[i].upstream_mtu > 0) r->upstream_mtu = results[i].upstream_mtu;
            else r->upstream_mtu = 512;
            
            if (results[i].down_mtu_search.optimal > 0) {
                r->downstream_mtu = results[i].down_mtu_search.optimal;
                if (r->downstream_mtu > g_cfg.max_download_mtu) r->downstream_mtu = g_cfg.max_download_mtu;
            } else {
    ...
    if (up_mtu_min < 9999) LOG_INFO("Upstream MTU range: %d - %d\n", up_mtu_min, up_mtu_max);
    if (down_mtu_min < 9999) LOG_INFO("Downstream MTU range: %d - %d\n", down_mtu_min, down_mtu_max);
