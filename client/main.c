...
#define LOG_WARN(...)                                                          \
  do {                                                                         \
    if (log_level() >= 1) {                                                    \
      ...
      if (g_debug_log)                                                         \
        fprintf(g_debug_log, "[WARN]  " __VA_ARGS__);                          \
      tui_debug_log(&g_tui, 1, __VA_ARGS__);                                   \
    }                                                                          \
  } while (0)
#define LOG_ERR(...)                                                           \
  do {                                                                         \
...
static uint8_t get_unused_session_id(void) {
  ...
  for (int i = 1; i <= 256; i++) {
    ...
    for (int j = 0; j < DNSTUN_MAX_SESSIONS; j++) {
      if (g_sessions[j].established && !g_sessions[j].closed &&
          g_sessions[j].session_id == sid) {
        in_use = true;
        break;
      }
    }
...
static void resolvers_load(void) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    uv_mutex_lock(&g_pool.lock);
    for (int i = 0; i < g_pool.count; i++) {
      if (strcmp(g_pool.resolvers[i].ip, line) == 0) {
        dup = 1;
        break;
      }
    }
...
static int build_dns_query(uint8_t *outbuf, size_t *outlen,
                           const chunk_header_t *hdr, const uint8_t *payload,
                           size_t paylen, const char *domain) {
/* Encode header + payload into a single base32 blob */
/* Extended header is 20 bytes (with OTI + extended chunk_info), payload can be
 * up to DNSTUN_CHUNK_PAYLOAD */
#define HDR_AND_PAYLOAD_MAX (20 + DNSTUN_CHUNK_PAYLOAD)
  ...
  if (payload && paylen > 0) {
    ...
    if (paylen > DNSTUN_CHUNK_PAYLOAD) {
      LOG_ERR(
          "Payload too large: %zu bytes (max %d). FEC symbol size mismatch!\n",
...
#define BASE32_MAX_OUTPUT(max_input) (((max_input) * 8 + 4) / 5)
  ...
  LOG_INFO("DEBUG: build_dns_query - rawlen=%zu (sizeof(chunk_header_t)=%zu, "
           "paylen=%zu)\n",
...
#define MAX_PAYLOAD_FOR_DNS (137 - (int)sizeof(chunk_header_t))
    if (DNSTUN_CHUNK_PAYLOAD > MAX_PAYLOAD_FOR_DNS) {
      LOG_ERR("DNSTUN_CHUNK_PAYLOAD (%d) too large for DNS limit with 20-byte "
              "header. Max: %d\n",
   ...
   * New compact header doesn't include session_id in QNAME - it's in the flags
   * byte CRITICAL: Must end with trailing dot for FQDN format! No delimiter
   * needed - server strips known domain suffix to extract payload
   *
  ...
  int qname_len =
      snprintf(qname, sizeof(qname), "%s.%s.", b32_dotted, clean_domain);
...
typedef struct {
  ...
  uint16_t
      upstream_mtu; /* EDNS payload size from Phase 3 / Binary search result */
...
typedef struct probe_req {
  ...
  resolver_test_result_t
      *result; /* Pointer to shared result for this resolver */
...
static void on_debug_recv(uv_udp_t *h, ssize_t nread, const uv_buf_t *buf,
      ...
      for (int i = 0; i < (int)resp->ancount; i++) {
        ...
        if (ans->generic.type == RR_TXT && ans->txt.len > 0) {
          ...
          ptrdiff_t decoded_len =
              base64_decode(decoded_payload, ans->txt.text, ans->txt.len);
          ...
          if (decoded_len > 0) {
            ...
            if ((size_t)decoded_len >=
                    payload_start + strlen(d->expected_payload) &&
                memcmp(decoded_payload + payload_start, d->expected_payload,
                       strlen(d->expected_payload)) == 0) {
              /* Success! */
...
void send_debug_packet(const char *payload, uint32_t seq) {
  LOG_INFO(
      "DEBUG: send_debug_packet called with payload='%s' (%zu bytes), seq=%u\n",
  ...
  const char *domain =
      (g_cfg.domain_count > 0) ? g_cfg.domains[0] : "tun.example.com";
  LOG_INFO("DEBUG: send_debug_packet calling build_dns_query - payload='%s', "
           "paylen=%zu, domain='%s'\n",
           payload, strlen(payload), domain);

  d->sendlen = sizeof(d->sendbuf);
  if (build_dns_query(d->sendbuf, &d->sendlen, &hdr, (const uint8_t *)payload,
                      strlen(payload), domain) != 0) {
    free(d);
    tui_proto_test_on_timeout(&g_tui);
    return;
  }
  ...
  uv_udp_send(&d->send_req, &d->udp, &buf, 1, (const struct sockaddr *)&d->dest,
              on_debug_send);
}
...
static void on_probe_recv(uv_udp_t *h, ssize_t nread, const uv_buf_t *buf,
                          const struct sockaddr *addr, unsigned flags) {
  if (nread == 0 && addr == NULL)
    return; /* spurious wake-up, ignore */
  (void)flags;
  probe_req_t *p = h->data;
  ...
  if (nread > 0) {
    ...
    if (p->test_type == PROBE_TEST_CRYPTO_CHALLENGE && p->result) {
      ...
      if (nread > 12) {
        ...
        if (rcode == 0 && ancount > 0) {
          ...
          LOG_WARN(
              "Resolver %d: Crypto challenge FAILED (rcode=%u, ancount=%u)\n",
       ...
       * resp.rcode() in (dns.rcode.NXDOMAIN, dns.rcode.NOERROR) and
       * len(resp.answer) == 0 Fake resolvers return REFUSED (5) or give wrong
       * answers */
      if (nread >= 12) {
        uint8_t *resp = (uint8_t *)buf->base;
       ...
       * The key insight: if we got ANY response, the resolver processed our
       * query. EDNS is supported if there's an OPT record (type 41) in the
       * response */
      ...
      if (nread >= 12) {
        ...
        while (offset + 11 <= (size_t)nread && visited_count < MAX_VISITED) {
          ...
          if (offset + 11 + rdlen > (size_t)nread)
            break;
          offset += 11 + rdlen;
        }
        ...
        if (p->result->edns_supported || p->result->txt_supported) {
          ...
          p->result->txt_supported =
              true; /* Resolver processed the TXT query */
          p->got_reply = true;
        }
      }
    } else if ((p->test_type == PROBE_TEST_MTU_UP ||
                p->test_type == PROBE_TEST_MTU_DOWN) &&
               p->result) {
      /* Phase 4: MTU Binary Search Response Handler
       * For upload tests: Check if we got a valid response at the requested MTU
       * size For download tests: Check if server can send a response of the
       * requested size Success means the MTU value works, failure means it's
       * too large */
      ...
      if (nread >= 12) {
        ...
        /* RCODE 0 (NOERROR) or 3 (NXDOMAIN) means the resolver processed our
         * query For download MTU tests, we care if the packet got through If we
         * got any valid DNS response, the packet was not dropped by the path */
        if (rcode == 0 || rcode == 3) {
          mtu_success = true;

          /* For upload MTU tests, also check if the resolver echoed our EDNS
           * payload size This tells us the actual MTU the resolver will accept
           */
          if (p->test_type == PROBE_TEST_MTU_UP && nread > 12) {
            ...
            while (offset + 11 <= (size_t)nread) {
              ...
              if (name == 0 && rtype == 41) { /* OPT record - EDNS */
                ...
                uint16_t resolver_payload =
                    (resp[offset + 3] << 8) | resp[offset + 4];
          ...
          /* For download MTU tests, we can't directly measure packet size
           * acceptance from the response alone. We rely on receiving any
           * response as success, and the binary search will narrow down based
           * on failures */
        }
      }
      ...
      if (p->test_type == PROBE_TEST_MTU_UP) {
        mark_mtu_tested(&p->result->up_mtu_search, p->mtu_under_test,
                        mtu_success);
        if (mtu_success && p->result->up_mtu_search.optimal == 0) {
          /* Record successful MTU (unless we already got resolver's payload
           * from above) */
          p->result->up_mtu_search.optimal = p->mtu_under_test;
        }
        ...
        mark_mtu_tested(&p->result->down_mtu_search, p->mtu_under_test,
                        mtu_success);
      ...
      if (p->test_type == PROBE_TEST_MTU_UP) {
        ...
        if (next_mtu > 0) {
          fire_mtu_test_probe(p->resolver_idx, PROBE_TEST_MTU_UP, p->result,
                              next_mtu);
        }
      } else {
        next_mtu = get_next_mtu_to_test(&p->result->down_mtu_search);
        if (next_mtu > 0) {
          fire_mtu_test_probe(p->resolver_idx, PROBE_TEST_MTU_DOWN, p->result,
                              next_mtu);
        }
      }
    } else {
    ...
    /* MTU test failed (timeout/no response) - mark as failed and continue
     * search */
    if ((p->test_type == PROBE_TEST_MTU_UP ||
         p->test_type == PROBE_TEST_MTU_DOWN) &&
        p->result) {
...
static void fire_probe_ext(int idx, const uint8_t *payload, size_t paylen,
                           const char *domain) {
  ...
  if (build_dns_query(p->sendbuf, &p->sendlen, &hdr, payload, paylen, domain) !=
      0) {
    free(p);
    return;
  }
  ...
  uv_udp_send(&p->send_req, &p->udp, &buf, 1, (const struct sockaddr *)&p->dest,
              on_probe_send);
}
...
static size_t build_test_dns_query(uint8_t *buf, size_t bufsize,
  ...
  while (*p) {
    ...
    if (offset + label_len + 1 > bufsize - (use_edns ? 11 : 0))
      break;
 ...
 * - Downstream: Use mtu-req-[N].tun.domain.com format so server sends large
 * response Returns total packet size */
static size_t build_mtu_test_query(uint8_t *buf, size_t bufsize,
  ...
  if (is_upload && target_mtu > 0) {
     ...
     * Where padding is random base32-compatible characters split into 63-char
     * labels
     *
    ...
    size_t padding_needed =
        (target_mtu > (int)overhead) ? (target_mtu - (int)overhead) : 0;
    ...
    while (padding_needed > 0) {
      ...
      if (offset + 1 + label_len + 1 > bufsize - 64)
        break;
    ...
    while (*p) {
      ...
      if (!dot)
        dot = p + strlen(p);
      size_t label_len = dot - p;
      if (offset + label_len + 1 > bufsize - 64)
        break;
      buf[offset++] = (uint8_t)label_len;
      memcpy(buf + offset, p, label_len);
      offset += label_len;
  ...
  uint16_t udp_size =
      (target_mtu > 0 && target_mtu < 1400) ? (uint16_t)target_mtu : 1232;
...
static void fire_test_probe(int idx, probe_test_type_t test_type,
  ...
  case PROBE_TEST_CRYPTO_CHALLENGE:
     ...
     * Format: "CRYPTO_<random_32_bytes_hex>" */
    {
      ...
      const char *base =
          g_cfg.test_domain[0] ? g_cfg.test_domain : "s.domain.com";
  ...
  case PROBE_TEST_LONGNAME:
    ...
    domain = g_cfg.long_label_domain[0] ? g_cfg.long_label_domain
                                        : "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                                          "aaaaaaaaaaaaaaaaaaaa.google.com";
    qtype = 1; /* A record */
    use_edns = false;
    break;
  ...
  p->sendlen = build_test_dns_query(p->sendbuf, sizeof(p->sendbuf), domain,
                                    qtype, 1, use_edns);
...
static void init_mtu_binary_search(mtu_binary_search_t *bs, int min_mtu,
                                   int max_mtu, int min_threshold,
                                   int allowed_min_mtu, int retries,
...
static bool is_mtu_tested(mtu_binary_search_t *bs, int mtu) {
  if (!bs->tested_cache || mtu < 0 || mtu >= bs->cache_size * 8)
    return false;
...
static int get_next_mtu_to_test(mtu_binary_search_t *bs) {
  ...
  int mid = (bs->optimal > 0) ? ((bs->optimal + bs->low) / 2)
                              : ((bs->high + bs->low) / 2);
...
static void fire_mtu_test_probe(int idx, probe_test_type_t test_type,
  ...
  p->sendlen = build_mtu_test_query(p->sendbuf, sizeof(p->sendbuf), domain, 16,
                                    mtu_size, is_upload);
  ...
  uv_timer_start(&p->timer, on_probe_timeout,
                 (uint64_t)g_cfg.mtu_test_timeout_ms > 0
                     ? g_cfg.mtu_test_timeout_ms
                     : 1000,
                 0);
...
static bool agg_packet_add_symbol(agg_packet_t *pkt, const uint8_t *symbol,
  ...
  if (!pkt || !symbol)
    return false;
  if (pkt->symbol_count >= DNSTUN_MAX_SYMBOLS_PER_PACKET)
    return false;
  if (symbol_size > DNSTUN_SYMBOL_SIZE)
    symbol_size = DNSTUN_SYMBOL_SIZE;
...
static size_t encode_aggregated_packet(uint8_t *out_buf, size_t out_size,
  ...
  if (!out_buf || !data || data_len == 0 || mtu == 0)
    return 0;

  int symbol_size =
      g_cfg.symbol_size > 0 ? g_cfg.symbol_size : DNSTUN_SYMBOL_SIZE;
  size_t header_size = sizeof(chunk_header_t);

  if (out_size < header_size)
    return 0;
...
static int decode_aggregated_packet(uint8_t *symbols[], uint8_t sizes[],
                                    ...
                                    int max_symbols) {
  if (!packet || packet_len < sizeof(chunk_header_t) + 1)
    return 0;
  ...
  if (symbol_count > DNSTUN_MAX_SYMBOLS_PER_PACKET)
    symbol_count = DNSTUN_MAX_SYMBOLS_PER_PACKET;
    ...
    if (i * symbol_size < payload_len) {
      ...
      if (sizes) {
        sizes[i] = (uint8_t)((i + 1) * symbol_size <= payload_len)
                       ? symbol_size
                       : (uint8_t)(payload_len - (i * symbol_size));
      }
    }
  }
...
static void log_aggregation_stats(void) {
  ...
  for (int i = 0; i < g_pool.count; i++) {
    ...
    if (r->state == RSV_ACTIVE && r->upstream_mtu > 0) {
      ...
      double efficiency =
          calc_packing_efficiency(r->upstream_mtu, optimal_size);
...
static void resolver_init_phase(void) {
  ...
  /* Filter: keep only resolvers with correct NXDOMAIN behavior (with
   * relaxation) */
  int nxdomain_ok = 0;
  for (int i = 0; i < g_pool.count; i++) {
    if (results[i].longname_supported && !results[i].nxdomain_correct) {
      /* [RELAXATION] Fake resolvers (hijacked DNS) are common but still usable
         for tunneling if the domain exists. Just log it. */
      LOG_WARN(
  ...
  for (int i = 0; i < g_pool.count; i++) {
    ...
    if (results[i].longname_supported && results[i].nxdomain_correct &&
      ...
      /* Passed Phase 1 and 2, but failed Phase 3 - restrict to minimal MTU and
       * proceed */
  ...
  for (int i = 0; i < g_pool.count; i++) {
    ...
    if (r->state != RSV_ACTIVE) {
      ...
      LOG_INFO("Resolver %s state=%s UpMTU=%u DownMTU=%u RTT=%.1fms\n", r->ip,
               state_str, r->upstream_mtu, r->downstream_mtu, r->rtt_ms);
    }
  }
...
/* Fire one DNS query chunk for a session. Falls back to dead resolvers if
 * needed. */
static void fire_dns_chunk_symbol(int session_idx, uint16_t seq,
                                  const uint8_t *payload, size_t paylen,
                                  int total_chunks, uint64_t oti_common,
                                  uint32_t oti_scheme);
...
static inline bool is_within_window(uint16_t seq, uint16_t expected,
                                    int window) {
 ...
 * A negative value means the packet is behind (already processed or wrapped
 * around). */
static int reorder_buffer_find_slot(reorder_buffer_t *rb, uint16_t seq) {
  /* Use signed 16-bit arithmetic to handle wrap-around correctly (distance <=
   * 32767) */
...
static bool reorder_buffer_insert(reorder_buffer_t *rb, uint16_t seq,
  ...
  if (offset < 0) {
    ...
    LOG_DEBUG("Reorder: dropping old/duplicate packet seq=%u (expected=%u, "
              "diff=%d)\n",
  ...
  if (offset >= RX_REORDER_WINDOW) {
    /* Packet is outside our window. Since offset is positive, it's in the
     * future. Only jump FORWARD if the gap is reasonably small (e.g. within
     * 32768 range). */
    if (offset < 32768) {
      LOG_INFO(
          "Reorder: jumping expected_seq forward from %u to %u (gap of %d)\n",
      ...
      LOG_DEBUG("Reorder: dropping out-of-range packet seq=%u (expected=%u, "
                "diff=%d)\n",
                seq, rb->expected_seq, offset);
      return false;
    }
...
static int reorder_buffer_flush(reorder_buffer_t *rb, uint8_t *out_buf,
  ...
  *out_len = 0;
  ...
  while (rb->slots[0]
             .valid) { /* Continue while we have the next expected packet */
...
static size_t socks5_handle_data(socks5_client_t *c, const uint8_t *data,
                                 size_t len) {
  ...
  if (c->state == 1) {
    ...
    if (atype == 0x01) {
      ...
      if (len < 5)
        return 0; /* Need at least dlen byte */
      uint8_t dlen = data[4];
    ...
    if (data[1] != 0x01) { /* Only CONNECT is supported */
      uint8_t err[10] = {0x05, 0x07, 0x00, 0x01, 0,
                         0,    0,    0,    0,    0}; /* Command not supported */
      socks5_send(c, err, 10);
      uv_close((uv_handle_t *)&c->tcp, on_socks5_close);
      return min_len;
    }
    ...
    if (atype == 0x01) { /* IPv4 */
      snprintf(sess->target_host, sizeof(sess->target_host), "%d.%d.%d.%d",
               data[4], data[5], data[6], data[7]);
      sess->target_port = (uint16_t)((data[8] << 8) | data[9]);
    } else if (atype == 0x03) { /* Domain */
      uint8_t dlen = data[4];
      if (dlen >= sizeof(sess->target_host))
        return min_len;
     ...
     * The server needs this to parse the target and establish upstream
     * connection. */
    if (min_len > 0) {
     ...
     * Standard SOCKS5 clients (like curl) wait for our "Success" reply before
     * sending data. Previously, we waited for a server DNS ACK, which often
     * timed out curl. Now we reply 'connected' immediately to pull the user
     * data into our buffer. */
    uint8_t reply[10] = {0x05, 0x00, 0x00, 0x01, 0, 0, 0, 0, 0, 0};
    socks5_send(c, reply, 10);
    sess->socks5_connected = true;

    LOG_INFO("SOCKS5 state 1 -> 2 (Tunnel). Consumed %zu bytes. (Optimistic "
             "ACK sent)\n",
             min_len);
    return min_len;
  }
  ...
  if (c->state == 2) {
    ...
    if (new_len > sess->send_cap) {
      ...
      if (sess->send_len >= MAX_SESSION_BUFFER) {
        ...
        memmove(sess->send_buf, sess->send_buf + drop_len,
...
static void on_socks5_read(uv_stream_t *s, ssize_t nread, const uv_buf_t *buf) {
  ...
  if (nread == 0)
    return; /* EAGAIN or similar, ignore */
  ...
  /* Process accumulated data in loop - handshake may complete across multiple
   * reads */
  while (c->buf_len > 0) {
...
static void on_socks5_alloc(uv_handle_t *h, size_t sz, uv_buf_t *buf) {
  socks5_client_t *c = h->data;
  (void)sz;
  /* Return pointer to end of current buffer content to avoid overwriting
   * existing data */
...
static void on_dns_recv(uv_udp_t *h, ssize_t nread, const uv_buf_t *buf,
  ...
  if (nread > 0) {
    fprintf(stderr,
            "[DEBUG] on_dns_recv: received %zd bytes (recvbuf size=%zu)\n",
      ...
      for (int i = 0; i < (int)resp->ancount; i++) {
        ...
        if (ans->generic.type == RR_TXT && ans->txt.len > 0) {
          ...
          if (is_sync) {
            ...
            /* Decode base64 response from server (server sends base64 by
             * default) */
            ...
            ptrdiff_t decoded_len =
                base64_decode(decoded, ans->txt.text, ans->txt.len);
            if (decoded_len < 0) {
              LOG_DEBUG("base64_decode FAILED for TXT len=%zu\n", ans->txt.len);
              continue;
            }

            LOG_DEBUG("DNS Data: Decoded=%td, RID=%d, starts with: %02x %02x "
                      "%02x %02x\n",
                      decoded_len, ridx, decoded[0], decoded[1], decoded[2],
                      decoded[3]);
             ...
             */
            {
              ...
              if (sidx >= 0 && sidx < DNSTUN_MAX_SESSIONS &&
                ...
                if (decoded_len >= sizeof(server_response_header_t)) {
                  ...
                  /* [Fix] Handle RESP_FLAG_STATUS: extract status byte AFTER
                   * capability header when server signals that status is
                   * prepended. Server sends: [cap_header (7 bytes)] [status (1
                   * byte)] [data...] Client needs to skip cap_header (7 bytes),
                   * then extract status. */
                  bool has_status = (hdr.flags & RESP_FLAG_STATUS) != 0;
                  if (has_status) {
                    const size_t cap_header_size =
                        sizeof(capability_header_t); /* 7 bytes */
                    /* Extract status byte AFTER capability header */
                    if (payload_len >= cap_header_size + 1) {
                      uint8_t status_byte =
                          decoded[sizeof(hdr) + cap_header_size];
                      LOG_DEBUG(
                          "Session %d: extracted status byte %02x from response "
                          "(after %zu-byte cap header)\n",
                          sidx, status_byte, cap_header_size);

                      /* [Fix] Reset SOCKS state on new handshake.
                       * This prevents stale state from previous sessions
                       * causing the client to mishandle the new SOCKS5
                       * handshake. */
                      if (!s->status_consumed) {
                        s->status_consumed = true; /* Mark as consumed */
                        s->socks5_connected =
                            false; /* Reset for new handshake */

                        /* Send SOCKS5 reply to client application */
                        if (s->client_ptr) {
                          ...
                          if (status_byte == 0x00) {
                            ...
                            LOG_INFO("Session %d: SOCKS5 success (status in "
                                     "response)\n",
                                     sidx);
                          } else {
                            /* Error: Send SOCKS5 error reply */
                            uint8_t err[10] = {
                                0x05, status_byte, 0x00, 0x01, 0,
                                0,    0,           0,    0,    0};
                            socks5_send(c, err, 10);
                            g_stats.socks5_total_errors++;
                            g_stats.socks5_last_error = status_byte;
                            LOG_WARN(
                                "Session %d: SOCKS5 error %02x from server\n",
                                sidx, status_byte);
                          }
                        }
                      }

                      /* Skip cap_header (7 bytes) + status (1 byte) for data
                       * processing */
                      payload = decoded + sizeof(hdr) + cap_header_size + 1;
                      payload_len =
                          (size_t)(decoded_len - sizeof(hdr) - cap_header_size -
                                   1);
                    }
                  } else if (has_seq) {
                    seq = hdr.seq;
                  ...
                  if (hdr.flags & RESP_FLAG_CLOSED) {
                    LOG_INFO("Session %d: server indicated session closed (FIN "
                             "received)\n",
                             sidx);
                    s->closed = true;
                    if (s->client_ptr) {
                ...
                if (has_seq) {
                  /* Always buffer sequenced packets in reorder window to
                   * prevent gaps */
                  reorder_buffer_insert(&s->reorder_buf, seq, payload,
                                        payload_len);

                  /* Drain all consecutive ready packets from the buffer */
                  uint8_t flush_buf[16384];
                  size_t flush_len = 0;
                  /* Loop while packets are being flushed from the reorder
                   * buffer */
                  while (reorder_buffer_flush(&s->reorder_buf, flush_buf,
                    ...
                    if (flush_len >= 1 && !s->status_consumed) {
                      ...
                      data_start = 1; /* Always skip the status byte; don't send
                                         to application */

                      if (s->client_ptr) {
                        socks5_client_t *c = (socks5_client_t *)s->client_ptr;
                        if (!s->socks5_connected) {
                          /* Non-optimistic mode or first reply: send status to
                           * app */
                          if (status_byte == 0x00) {
                            /* Success: Send SOCKS5 success reply (Address Type
                             * IPv4, 0.0.0.0:0) */
                            uint8_t ok[10] = {0x05, 0x00, 0x00, 0x01, 0,
                                              0,    0,    0,    0,    0};
                            socks5_send(c, ok, 10);
                            s->socks5_connected = true;
                            g_stats.socks5_last_error = 0;
                            LOG_INFO("Session %d: SOCKS5 success (ACK "
                                     "processed in sequence)\n",
                                     sidx);
                          } else {
                            /* Mapped Error: status_byte from server mapped to
                             * SOCKS5 reply field */
                            uint8_t err[10] = {
                                0x05, status_byte, 0x00, 0x01, 0,
                                0,    0,           0,    0,    0};
                           ...
                           * If server reports error now, we must close the
                           * connection. */
                          if (status_byte != 0x00) {
                            LOG_WARN("Session %d: Post-optimistic SOCKS5 error "
                                     "%02x from server, closing\n",
                                     sidx, status_byte);
                            g_stats.socks5_total_errors++;
                            g_stats.socks5_last_error = status_byte;
                            uv_close((uv_handle_t *)&c->tcp, on_socks5_close);
                          } else {
                            LOG_INFO("Session %d: SOCKS5 status 00 confirmed "
                                     "by server (optimistic ACK was correct)\n",
                                     sidx);
                          }
                        }
                      }
                    ...
                    if (data_len > 0) {
                      ...
                      if (need > s->recv_cap) {
                        size_t new_cap =
                            (need + 8191) & ~4095; /* Round up to 4KB */
                        if (new_cap > MAX_SESSION_BUFFER)
                          new_cap = MAX_SESSION_BUFFER;
                  ...
                  /* Emergency fallback for non-sequenced packets (should not
                   * occur) */
 ...
 * The handshake packet is 5 bytes: version(1) + upstream_mtu(2) +
 * downstream_mtu(2) Server detects it by: payload_len == 5 && payload[0] ==
 * DNSTUN_VERSION
 */
static void send_mtu_handshake(int session_idx) {
  session_t *sess = &g_sessions[session_idx];
  ...
  /* [Fix] Reset SOCKS5 state for new handshake.
   * This prevents stale state from previous sessions causing the client to
   * mishandle the new SOCKS5 handshake (e.g., ignoring the status byte
   * from the new session). */
  sess->status_consumed = false;
  sess->socks5_connected = false;
  sess->recv_len = 0;

  LOG_DEBUG("Session %d: reorder buffer and sequence counters reset for new "
            "handshake\n",
            session_idx);
  ...
  /* Send via the normal chunk path - server will detect handshake by
   * payload_len == 5 */
  fire_dns_chunk_symbol(session_idx, 0, hs_payload, sizeof(hs_payload), 0, 0,
                        0);
}
...
static void fire_dns_chunk_symbol(int session_idx, uint16_t seq,
                                  const uint8_t *payload, size_t paylen,
                                  int total_symbols, uint64_t oti_common,
                                  uint32_t oti_scheme) {
  ...
  if (ridx < 0) {
    LOG_ERR("fire_dns_chunk_symbol: no resolvers available at all "
            "(session_idx=%d, seq=%u)\n",
            session_idx, seq);
  ...
  chunk_header_t hdr = {0};
  hdr.flags =
      (g_cfg.encryption ? CHUNK_FLAG_ENCRYPTED : 0) | CHUNK_FLAG_COMPRESSED;
  if (paylen == 0)
    hdr.flags |= CHUNK_FLAG_POLL; /* poll flag */
  if (total_symbols > 0)
    hdr.flags |= CHUNK_FLAG_FEC; /* fec flag */
   ...
   * This tells the server how many symbols are needed to decode (k original
   * symbols). */
  ...
  /* The capability header tells the server the client's MTU/encoding
   * capabilities. CRITICAL: Do NOT prepend it to FEC data symbols. FEC symbols
   * are exactly DNSTUN_CHUNK_PAYLOAD (110 bytes). Adding 7 cap bytes into a
   * 110-byte buffer silently truncates the symbol to 103 bytes, breaking FEC
   * decoding on the server. The cap header is only sent on polls and non-FEC
   * packets (MTU handshake). */
  ...
  if (total_symbols > 0) {
    ...
    if (paylen > DNSTUN_CHUNK_PAYLOAD)
      paylen = DNSTUN_CHUNK_PAYLOAD;
    if (payload && paylen > 0)
      memcpy(payload_with_cap, payload, paylen);
    final_paylen = paylen;
  } else {
  ...
  if (build_dns_query(q->sendbuf, &q->sendlen, &hdr, payload_with_cap,
                      final_paylen, domain) != 0) {
    free(q);
    return;
  }
...
static void on_poll_timer(uv_timer_t *t) {
  ...
  for (int i = 0; i < DNSTUN_MAX_SESSIONS; i++) {
    ...
    if (!sess->established || sess->closed)
      continue;
    ...
    if (sess->client_ptr) {
      ...
      if (c->state == 2 && sess->send_len == 0 &&
          (time(NULL) - sess->last_active > 5)) {
        static time_t last_warn = 0;
        if (time(NULL) - last_warn > 10) {
          LOG_WARN(
              "Session %d: STUCK? Handshake OK but 0 bytes uploaded for 5s.\n",
              i);
          last_warn = time(NULL);
        }
      }
    ...
    if (sess->send_len > 0) {
      ...
      if (fec.total_count > 0) {
        ...
        for (int s = 0; s < fec.total_count; s++) {
          fire_dns_chunk_symbol(i, sess->tx_next++, fec.symbols[s],
                                fec.symbol_len, fec.total_count,
                                fec.has_oti ? fec.oti_common : 0,
                                fec.has_oti ? fec.oti_scheme : 0);
        }
      }

      codec_fec_free(&fec);
      if (g_cfg.encryption)
        codec_free_result(&eret);
      codec_free_result(&cret);
...
static void fire_chrome_cover_traffic(uv_timer_t *t) {
  (void)t;
  if (!g_cfg.chrome_cover)
    return;

  /* Mimic Chrome: A + AAAA queries for top domains */
  const char *sites[] = {"google.com", "youtube.com", "fonts.gstatic.com",
                         "ssl.gstatic.com"};
...
static void on_recovery_timer(uv_timer_t *t) {
  ...
  int probes[64];
  int n =
      rpool_dead_to_probe(&g_pool, probes, 64, g_cfg.background_recovery_rate);
  const char *domain =
      (g_cfg.domain_count > 0) ? g_cfg.domains[0] : "tun.example.com";
  for (int i = 0; i < n; i++) {
    int idx = probes[i];
...
int main(int argc, char *argv[]) {
  ...
  if (!config_path) {
    ...
    const char *candidates[] = {"client.ini", "../client.ini",
  ...
  if (uv_listen((uv_stream_t *)&g_socks5_server, 128, on_socks5_connection) !=
      0) {
  ...
  uv_timer_start(&g_poll_timer, on_poll_timer, g_cfg.poll_interval_ms,
                 g_cfg.poll_interval_ms);
