...
void dns_tx_send_raw(const uint8_t *buf, size_t len) {
    ...
    /* Global UDP handle from resolver_pool.c can be used, 
     * but here we use the specific resolver's address */
    struct sockaddr_in dest;
    uv_ip4_addr(r->ip, 53, &dest);
    
    /* Note: In a real system, we'd use a unified UDP server handle */
