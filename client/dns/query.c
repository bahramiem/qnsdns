...
#define HDR_AND_PAYLOAD_MAX (512)
  ...
  if (payload && paylen > 0) {
    if (rawlen + paylen > HDR_AND_PAYLOAD_MAX)
      return -1;
...
#define BASE32_MAX_OUTPUT(max_in) (((max_in) * 8 + 4) / 5)
  ...
  size_t sz = *outlen;
  if (dns_encode((dns_packet_t *)outbuf, &sz, &query) != RCODE_OKAY)
    return -1;
  *outlen = sz;
  return 0;
}
...
int fire_dns_multi_symbols(int session_idx, uint16_t seq,
  ...
  /* ── FEC SYNC CHECK ──
   * Only allow high-speed data bursts if the server has acknowledged FEC
   * parameters. This avoids the "Extracted symbol len 0" race on the server. */
  if (num_symbols > 1 || total_symbols_in_burst > 1) {
    if (!sess->fec_synced &&
        !g_cfg.encryption) { /* Allow encryption-only for testing, but FEC needs
                                MTU sync */
  ...
  do {
    ...
    if (num_symbols == 0) {
      ...
      cap.encoding =
          (r->enc == ENC_BASE64) ? DNSTUN_ENC_BASE64 : DNSTUN_ENC_HEX;
      ...
      /* HANDSHAKE: Prepend NOTHING. Payload will be the bare 13-byte handshake
       * struct. */
      DBGLOG("[UPSTREAM] Sending Handshake to resolver %s\n", r->ip);
    } else {
      /* DATA/FEC: Prepend ONLY 2-byte compact ACK to save precious DNS payload
       * space */
    ...
    if (num_symbols > 0) {
      for (int i = 0; i < to_pack; i++) {
        if (pack_len + 1 + sym_size > sizeof(pack_buf))
          break;
    ...
    if (num_symbols == 0) {
      q_flags =
          (to_pack == 1 && total_symbols_in_burst == 0) ? 0 : CHUNK_FLAG_POLL;
    } else if (total_symbols_in_burst > 1) {
      q_flags = (is_compressed ? CHUNK_FLAG_COMPRESSED : 0) |
                (g_cfg.encryption ? CHUNK_FLAG_ENCRYPTED : 0) | CHUNK_FLAG_FEC;
    } else {
      q_flags = (is_compressed ? CHUNK_FLAG_COMPRESSED : 0);
    }

    hdr.sid   = sess->session_id;
    hdr.flags = q_flags | CHUNK_FLAG_IS_TUNNEL;
    hdr.seq = seq;
